  ...
  const articles = [
    {
      ...
      body: [
        "Every professional freelance invoice must contain: your full name and contact details, the client name and billing info, a unique invoice number like INV-001, the issue date and payment due date, an itemized list of services with quantities and rates, subtotal, tax amount, total, and clear payment instructions.",
        "The most important rule: always set a due date. Invoices with a due date get paid 3x faster than those without. Net 14 is standard for most freelancers. Net 30 works for larger agencies.",
        "Once your invoice is ready, download it as a PDF from InvoiceHQ and send it immediately. Never wait until end of month. The sooner you invoice, the sooner you get paid.",
      ],
    },
    {
      num: "02",
      title: "Multi-Currency Invoicing: How to Bill International Clients",
      summary: "Working with clients from the US, UK, or UAE? Here is how to invoice them in their currency and receive money from anywhere in the world.",
      body: [
        "Always invoice in the currency your client is most comfortable with. US clients expect USD, European clients prefer EUR, and UAE clients use AED. InvoiceHQ supports 10+ currencies including USD, EUR, GBP, INR, PKR, AED, CAD, AUD, JPY, and CHF.",
        "On exchange rates: do not put conversion rates on the invoice itself. Invoice in their preferred currency and let your payment processor like Payoneer, Wise, or PayPal handle the conversion when you withdraw.",
        "Best platforms to receive international payments: Payoneer works in 200+ countries with low fees. Wise has the best exchange rates. PayPal is widely accepted but charges higher fees of 3 to 5 percent.",
      ],
    },
    {
      num: "03",
      title: "The Freelancer Guide to Setting Rates That Get Paid",
      summary: "Most freelancers underprice themselves by 40 to 60 percent. Use this proven formula to calculate your correct rate and stop leaving money on the table.",
      body: [
        "Use this formula: Minimum Rate = (Monthly Expenses + Desired Profit) divided by Billable Hours Per Month. Example: $2,000 expenses + $1,500 profit = $3,500 needed. At 80 billable hours, your floor is $43.75 per hour.",
        "Rate benchmarks for 2026: UI/UX Designers earn $50 to $150 per hour, Web Developers $60 to $200 per hour, Content Writers $0.10 to $0.50 per word, Social Media Managers $500 to $3,000 per month per client.",
        "How to raise your rates: give 30 days notice, frame it as an annual review, and raise rates for new clients first. If a client leaves over a modest rate increase, they were never a sustainable long-term client.",
      ],
    },
    {
      num: "04",
      title: "How to Follow Up on Unpaid Invoices (With Email Templates)",
      summary: "Chasing late payments is uncomfortable but necessary. These professional email templates will get you paid without damaging client relationships.",
      body: [
        "Follow this timing: Day 0 send invoice immediately. Three days before due date send a friendly reminder. One day after due date send a polite follow-up. Seven days overdue send a firm reminder. Fourteen days overdue send a final notice with late fee warning.",
        "Friendly reminder template: Hi Client, just a quick reminder that Invoice INV-001 for $Amount is due on Date. Please let me know if you have any questions. Best, Your Name.",
        "Always attach the original invoice PDF to every follow-up. Clients often claim they never received it. Prevention tip: require a 50 percent deposit before starting any new project.",
      ],
    },
    {
      num: "05",
      title: "Recurring Invoices: How to Lock In Reliable Retainer Income",
      summary: "Retainer clients who pay monthly are the backbone of a sustainable freelance business. Here is how to structure recurring invoices and predictable income.",
      body: [
        "A retainer is a fixed monthly fee a client pays for ongoing access to your services. It provides predictable income, removes the constant need to find new clients, and builds deeper client relationships.",
        "How to pitch a retainer: after a successful project say, would you be interested in a monthly retainer? For a fixed amount per month I can cover your specific needs on an ongoing basis.",
        "In InvoiceHQ select your billing frequency in the Recurring field. Weekly, Bi-weekly, Monthly, Quarterly, or Yearly. Your invoice PDF will show the recurrence badge so clients always know when to expect the next invoice.",
      ],
    },
    {
      num: "06",
      ...
      body: [
        "A Quote is sent BEFORE work begins. It tells the client what the work will cost. An Invoice is sent AFTER work is completed. It is a formal legally binding request for payment. A Receipt is issued AFTER payment is received confirming the transaction is complete.",
        "The correct workflow is: Quote, then client approves, then do the work, then Invoice, then client pays, then Receipt. Always follow this order for clean financial records.",
      ...
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        <div style={{ marginBottom: 48 }}>
          ...
          <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(28px, 5vw, 42px)", fontWeight: 700, color: "#fff", marginBottom: 12 }}>
            The Freelancer Invoice Playbook
          </h2>
        ...
        <div>
          {articles.map((a, i) => (
            <div key={i} style={{ borderTop: "1px solid " + (expanded === i ? G.goldBorder : G.border), background: expanded === i ? "#D4A84B0d" : "transparent", transition: "all 0.3s" }}>
              <div onClick={() => setExpanded(expanded === i ? null : i)} style={{ display: "flex", alignItems: "flex-start", gap: 20, padding: "24px 16px", cursor: "pointer" }}>
                <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 30, fontWeight: 700, color: expanded === i ? G.gold : "#2a2a2a", minWidth: 40, lineHeight: 1, transition: "color 0.3s" }}>{a.num}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 16, fontWeight: 600, color: "#fff", marginBottom: 5 }}>{a.title}</div>
                  <div style={{ fontSize: 13, color: G.muted, lineHeight: 1.6 }}>{a.summary}</div>
                </div>
                <div style={{ width: 30, height: 30, borderRadius: "50%", border: "1px solid " + (expanded === i ? G.goldBorder : G.border), display: "flex", alignItems: "center", justifyContent: "center", color: expanded === i ? G.gold : G.muted, fontSize: 20, transition: "all 0.3s", transform: expanded === i ? "rotate(45deg)" : "rotate(0)", flexShrink: 0 }}>+</div>
              </div>
              {expanded === i && (
                <div style={{ padding: "4px 16px 28px 76px", borderTop: "1px solid " + G.border }}>
                  {a.body.map((para, j) => (
                    <p key={j} style={{ fontSize: 14, color: G.subtle, lineHeight: 1.85, marginTop: 14 }}>{para}</p>
                  ))}
                </div>
              )}
            </div>
